<%@ Page Language="VB" Inherits="OboutInc.oboutAJAXPage" Debug="true" %>
...
<script runat="server">
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        ' creating the Template for Edit Button
        '------------------------------------------------------------------------
        Dim AddButtonTemplate As Obout.Grid.GridRuntimeTemplate
        AddButtonTemplate = New GridRuntimeTemplate()
        AddButtonTemplate.ID = "tplAddBtn"
        AddButtonTemplate.Template = New Obout.Grid.RuntimeTemplate()
        AddHandler CType(AddButtonTemplate.Template, Obout.Grid.RuntimeTemplate).CreateTemplate, AddressOf CreateAddButtonTemplate
        '------------------------------------------------------------------------ 
        
        grid1.TemplateSettings.NewRecord_TemplateId = AddButtonTemplate.ID
    ...
    Sub CreateAddButtonTemplate(ByVal sender As Object, ByVal e As Obout.Grid.GridRuntimeTemplateEventArgs)
        Dim oLiteral As Literal = New Literal()
        e.Container.Controls.Add(oLiteral)
        AddHandler CType(oLiteral, Literal).DataBinding, AddressOf DataBindAddButtonTemplate
    End Sub
    
    Public Sub DataBindAddButtonTemplate(ByVal sender As Object, ByVal e As EventArgs)
        Dim oLiteral As Literal = CType(sender, Literal)
        Dim oContainer As Obout.Grid.TemplateContainer = CType(oLiteral.NamingContainer, Obout.Grid.TemplateContainer)
    ...
    Sub CreateGrid()
        ...
        Dim myComm As OleDbCommand = New OleDbCommand("SELECT TOP 25 OrderID,ShipName,ShipCountry,FORMAT(OrderDate, ""mm/dd/yyyy"") AS OrderDateFormatted FROM Orders ORDER BY OrderID DESC", myConn)
        myConn.Open()
    ...
    Public Function LoadRecord(ByVal orderId As String) As String()
        ...
        Dim myComm As OleDbCommand = New OleDbCommand("SELECT TOP 25 *, FORMAT(OrderDate, ""mm/dd/yyyy"") AS OrderDateFormatted, FORMAT(RequiredDate, ""mm/dd/yyyy"") AS RequiredDateFormatted, FORMAT(ShippedDate, ""mm/dd/yyyy"") AS ShippedDateFormatted FROM Orders WHERE OrderId=@OrderID", myConn)
        myComm.Parameters.AddWithValue("@OrderID", orderId)
        myConn.Open()
        Dim myReader As OleDbDataReader = myComm.ExecuteReader()
        Dim sRecords(12) As String
               
        While myReader.Read()
            sRecords(0) = myReader("ShipName")
            sRecords(1) = myReader("ShipAddress")
            sRecords(2) = myReader("ShipCity")
            sRecords(3) = myReader("ShipRegion")
            sRecords(4) = myReader("ShipPostalCode")
            sRecords(5) = myReader("ShipCountry")
            sRecords(6) = myReader("OrderID")
            sRecords(7) = myReader("OrderDate")
            sRecords(8) = myReader("RequiredDate")
            sRecords(9) = myReader("ShippedDate")
            sRecords(10) = myReader("ShipVia")
            sRecords(11) = myReader("Sent")
        End While
        myConn.Close()
        Return sRecords
...
<body>
   ...
   <form id="form1" runat="server">
        <div>
            ...
            <obout:Flyout runat="server" ID="Flyout1" Align="RIGHT" Position="BOTTOM_RIGHT" CloseEvent="NONE"
                            ...
                            <table width="100%" height="100%" cellpadding="0" cellspacing="0">
                                ...
                                <tr>
                                    <td colspan="3" align="center">
                                        ...
                                        <input type="button" value="Cancel" onclick="<%=Flyout1.getClientID()%>.Close()"
                                            class="tdText" />
                                    </td>
                                </tr>
                            </table>
            ...
            <obout:Flyout runat="server" ID="Flyout2" AttachTo="btAdd" Align="LEFT" Position="ABSOLUTE"
                ...
                <table class="rowEditTable">
                    ...
                    <tr>
                        <td colspan="3" align="center">
                            ...
                            <input type="button" value="Cancel" onclick="<%=Flyout2.getClientID()%>.Close()"
                                class="tdText" />
                        </td>
                    </tr>
                </table>
